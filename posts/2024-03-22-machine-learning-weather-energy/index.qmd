...
In Elastic Net Regression models, we can tune the `penalty` and `mixture` hyperparameters, which specify the amount of regularization and the mix of lasso and ridge penalties, respectively. I tuned my Elastic Net Regression models for 10 levels of `penalty` between 0.01 to 3 and 10 levels of `mixture` between 0 to 1. When `mixture = 0`, the model is actually performing a ridge regression and when `mixture = 1`, the model is performing a lasso regression.
